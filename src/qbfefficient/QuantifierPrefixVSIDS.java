...
public class QuantifierPrefixVSIDS {
	private String lm = new String("QUANTIFIER_BLOCK");
	...
	public void remove(int q) {
		if (!this.normalized) MyLog.log(lm, 0, "cannot remove quantifier when norlalized is false");
		if (q < 0) q = -q;
		int d = this.depth[q];
    ...
    public void insert(int q) {
    	if (!this.normalized) MyLog.log(lm, 0, "cannot call this insertion method when norlalized is false");
    	if (q < 0) q = -q;
    	int d = this.depth[q];
	...
	public Quantifier peek() {
		if (!this.normalized) MyLog.log(lm, 0, "cannot peek quantifier when norlalized is false");
		int i;
		...
		if (ret == null) MyLog.log(lm, 0, "invariant broken, no quantifier remaining");
	...
	public void updateWeight(int q, double v) {
		if (!this.normalized) MyLog.log(lm, 0, "cannot update VSIDS score when norlalized is false");
		if (q < 0) q = -q;
		this.remove(q);
		this.score[q] = v;
		this.insert(q);
